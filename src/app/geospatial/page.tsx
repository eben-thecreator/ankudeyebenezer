...
export default async function GeospatialPage() {
  ...
  const res = await fetch(`${baseUrl}/data/geospatial.json`, { cache: 'no-store' });
    ...
    <main className="w-full">
      ...
      <section className="relative w-full h-96 overflow-hidden bg-black">
        ...
        <div className="relative z-10 h-full flex flex-col items-start justify-end px-6 sm:px-12 pb-12">
          <h1 className="text-5xl sm:text-6xl font-extrabold text-white mb-3">GEOSPATIAL ENGINEERING</h1>
          <p className="text-lg text-gray-300 max-w-2xl">Spatial data modelling, GIS analysis, and mapping for precise decision-making and infrastructure planning.</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {projects.map((p: any, idx: number) => (
              <Link key={p.id} href={`/geospatial/${p.slug}`}>
                <article
